...
from ..realtime_triggers.triggers import Trigger
...
def realtime(
    ...
    if annot_csv_filename is None:
        annot_csv_filename = Path(output_dir).stem + constants.ANNOT_CSV_SUFFIX
    annot_csv_path = Path(output_dir).joinpath(annot_csv_filename)
    logger.info(f"will save annotations in .csv file: {annot_csv_path}")

    spect_queue = queue.Queue() # Is fed by the '_generate_spect' function.
    syllables_list = [] # Is fed by the '_predict' function. It's thread-safe as there is only 1 writer.
    # Run as long as there are triggers. TODO: also stop when the audio stream has ended (only after waiting for the last spects and predictions).
    spectrogram_thread = threading.Thread(target=_generate_spect, args=())
    predict_thread = threading.Thread(target=_predict, args=(
        spect_queue, 
        syllables_list,
        triggers,
        checkpoint_path,
        ...
        majority_vote,
        save_net_outputs))
    trigger_thread = threading.Thread(target=_trigger, args=(triggers, syllables_list))
...
def _generate_spect():
    """Reads the audio stream and creates spectrograms from it.
    """
    pass
...
def _predict(
    ...
    majority_vote,
    save_net_outputs):
    """Fills a growing list of predictions based of the spectrograms queued.
    """
    dataset_df = pd.read_csv(csv_path) # TODO: Convert to reading from the queue.
...
def _mock_predict(spect_queue, syllables_list):
    for i in range(10000):
        time.sleep(0.01)
        syllables_list.append(choice(['a','b','c','d','e','f','g','h','i']))

def _trigger(triggers, syllables_list):
    syllables_list_index = 0 # Rolling index - looking for triggeres from this point onwards.

    while triggers: # Stops when no triggers are left.
        if not syllables_list or len(syllables_list) == syllables_list_index:
            continue # If no new sylables - continue.
        for trig in triggers:
            trigger_index = trig.check_trigger(syllables_list[syllables_list_index:])
            if trigger_index != Trigger.NOT_TRIGGERED:
                # Todo - Should a trigger erase the current stream?
                # Todo - Should a trigger be 'single use'?
                triggers.remove(trig)
                syllables_list_index = trigger_index
    return
    ...
    # TODO: Should I add my own defaults mode?
